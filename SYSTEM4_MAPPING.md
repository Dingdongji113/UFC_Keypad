...
- ADF remains visibly ordered `1`, `OFF`, `2`, while its end command values are reversed to `2`, `1`, `0` to match the live cockpit direction.
- ALR-67 follows the physical left-to-right order `BIT`, `OFFSET`, `SPECIAL`, `DISPLAY`, `POWER`. Each square touch/annunciator face has fixed lamp rows: `FAIL/BIT`, `ENABLE/OFFSET`, `ENABLE/SPECIAL`, and `LIMIT/DISPLAY`; inactive rows remain blank. `FAIL` is red. POWER is a single-row exception that shows `ON` only while powered.
